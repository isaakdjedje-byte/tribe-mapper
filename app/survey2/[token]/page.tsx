...
const SURVEY2_QUESTIONS = [
  ...
  { id: 'values_2', section: 'Follow-up', question: 'When did you feel least aligned?', type: 'textarea', placeholder: 'A specific moment...', required: false },
...
export default function Survey2Page({ params }: { params: Promise<{ token: string }> }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const responses = Object.entries(answers).map(([question_id, answer_value]) => ({ question_id, answer_value, answer_type: SURVEY2_QUESTIONS.find(q => q.id === question_id)?.type || 'text' })).filter(r => r.answer_value !== undefined);
  ...
  if (completed) return <div className="container"><div className="card" style={{ textAlign: 'center', padding: 'var(--space-8)' }}><h2>Thank you</h2><p className="text-muted mt-3">Your additional insights are valuable.</p></div></div>;

  const q = SURVEY2_QUESTIONS[currentQuestion];
  const progress = ((currentQuestion + 1) / SURVEY2_QUESTIONS.length) * 100;

  return (
    <div className="container">
      <div className="progress">
        <div className="progress-header"><span className="progress-label">Follow-up Questions</span><span className="progress-count">{currentQuestion + 1} / {SURVEY2_QUESTIONS.length}</span></div>
      ...
      <div className="card" style={{ padding: 'var(--space-6)' }}>
        ...
        <div className="flex justify-between" style={{ marginTop: 'var(--space-6)' }}>
          ...
          {currentQuestion === SURVEY2_QUESTIONS.length - 1 ? (
            <button className="btn btn-primary" onClick={handleSubmit} disabled={isSubmitting}>{isSubmitting ? 'Submitting...' : 'Submit'}</button>
          ) : (
            <button className="btn btn-primary" onClick={() => setCurrentQuestion(p => p + 1)}>Continue</button>
          )}
        </div>
      </div>
    </div>
