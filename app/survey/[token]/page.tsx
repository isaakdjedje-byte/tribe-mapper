...
export default function SurveyPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const canProceed = () => {
    const q = SURVEY1_QUESTIONS[currentQuestion];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const responses = Object.entries(answers).map(([question_id, answer_value]) => ({ question_id, answer_value, answer_type: SURVEY1_QUESTIONS.find(q => q.id === question_id)?.type || 'text' }));
  ...
  if (isLoading) return <div className="container"><div className="card" style={{ textAlign: 'center', padding: 'var(--space-10)' }}><p className="text-muted">Loading...</p></div></div>;
  if (completed) return <div className="container"><div className="card" style={{ textAlign: 'center', padding: 'var(--space-8)' }}><h2>Thank you</h2><p className="text-muted mt-3">Your responses have been recorded.</p></div></div>;

  if (showConsent) return (
    <div className="container">
      <div className="card">
        <h2 style={{ marginBottom: 'var(--space-3)' }}>Help Us Understand Your Group</h2>
        <p className="text-secondary">This survey maps relationships, roles, and dynamics to help the group work better together.</p>
        <div className="consent">
          <label className="consent-item"><input type="checkbox" className="checkbox" checked={consentGiven} onChange={(e) => setConsentGiven(e.target.checked)} /><span>Your responses are confidential and used only for group analysis.</span></label>
          <label className="consent-item"><input type="checkbox" className="checkbox" /><span>You can skip any question or withdraw at any time.</span></label>
          <label className="consent-item"><input type="checkbox" className="checkbox" /><span>Individual responses won't be shared with other members.</span></label>
        </div>
        <button className="btn btn-primary" style={{ width: '100%' }} onClick={() => setShowConsent(false)} disabled={!consentGiven}>Continue</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="container">
      <div className="progress">
        <div className="progress-header"><span className="progress-label">{q.section}</span><span className="progress-count">{currentQuestion + 1} / {SURVEY1_QUESTIONS.length}</span></div>
      ...
      <div className="card" style={{ padding: 'var(--space-6)' }}>
        ...
        <div style={{ marginTop: 'var(--space-5)' }}>
          ...
          {q.type === 'nominate' && (
            <div>
              <p className="hint" style={{ marginBottom: 'var(--space-3)' }}>Select up to {q.maxSelections}</p>
        ...
        <div className="flex justify-between" style={{ marginTop: 'var(--space-6)' }}>
          <button className="btn btn-secondary" onClick={() => setCurrentQuestion(p => p - 1)} disabled={currentQuestion === 0}>Back</button>
          {currentQuestion === SURVEY1_QUESTIONS.length - 1 ? (
            <button className="btn btn-primary" onClick={handleSubmit} disabled={!canProceed() || isSubmitting}>{isSubmitting ? 'Submitting...' : 'Submit'}</button>
          ) : (
            <button className="btn btn-primary" onClick={() => setCurrentQuestion(p => p + 1)} disabled={!canProceed()}>Continue</button>
          )}
        </div>
      </div>
    </div>
